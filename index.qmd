...
# Basic beeswarm plot for df_se
beeswarm(score ~ grader, data = df_se,
         pch = 21,                                 # Use filled circles with border
         col = "black",                            # Outline color
         bg = viridis::viridis(length(unique(df_se$grader))), # 
         method="hex",
...
for(i in seq_along(meds)) {
  lines(c(i - 0.3, i + 0.3), rep(meds[i], 2), col = "red", lwd = 3)
}
```
