...
```{r}
ggplot(df_se, aes(x = score)) +
  geom_dotplot(binwidth = 2, dotsize = 0.8, fill = "blue", color = "black") +
  facet_wrap(~fig) +
  theme_grey() +
  labs(title = "Distribution of Scores (Dotplot)", x = "Score", y = "Count")
```

##

```{r}
# ggplot(df_se, aes(x = fig, y = score)) +
#   geom_point(color = "blue", alpha = 0.7, size = 2, position = position_jitter(width = 0.2, height = 0)) +
#   theme_grey() +
  # labs(title = "Distribution of Scores by Figure", x = "Figure", y = "Score")
```
